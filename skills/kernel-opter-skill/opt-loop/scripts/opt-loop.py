...
1. Snapshot the current kernel implementation into a run directory.
...
BACKEND_REFERENCE_DOCS = {
    "cuda": [
        "skills/kernel-opter-skill/SKILL.md",
        "skills/kernel-opter-skill/cuda/reference/memory-opt.md",
        "skills/kernel-opter-skill/cuda/reference/compute-opt.md",
        "skills/kernel-opter-skill/cuda/reference/latency-opt.md",
    ],
}
...
# Reasons that indicate infrastructure failure rather than code quality.
# Fingerprints rejected for these reasons should NOT be permanently blocked —
# the same optimization idea may succeed once the infrastructure issue is fixed.
...
def collect_preflight(
    ...
    add_requirement(
        requirements, errors, "solution suffix",
        solution_file.suffix.lower() == ".cu", solution_file.suffix or "(no suffix)",
    )
    add_requirement(requirements, errors, "benchmark.py", benchmark_script.exists(), str(benchmark_script))
    add_requirement(
        requirements, errors, "reference file",
    ...
    add_requirement(
        requirements, errors, "PyTorch import", torch_info["importable"],
        torch_info["version"] if torch_info["importable"] else (torch_info["error"] or "torch import failed"),
    )
    add_requirement(
        requirements, errors, "CUDA runtime", torch_info["cuda_available"],
    ...
    add_requirement(
        requirements, errors, f"GPU index {args.gpu}", selected_gpu_ok,
    ...
    add_requirement(
        requirements, errors, "nvcc executable", nvcc_info["exists"],
    ...
    add_requirement(
        requirements, errors, "ncu executable", ncu_info["exists"],
...
def build_scope_key(canonical_source_file: Path, ref_file: Path | None, dims_args: list[str], arch: str) -> str:
    ...
    return f"cuda__src_{source_token}__{ref_token}__{arch_token}__{dims_token}"
...
def default_strategy_memory(global_file: Path, scope_key: str) -> dict[str, Any]:
    return {
        ...
        "current_run": {"seen_order": [], "positive": {}, "negative": {}, "rejected": {}},
        "global_sync": {
            "enabled": True,
...
def build_strategy_fingerprint(tags: list[str]) -> str:
    canonical = {"backend": "cuda", "tags": tags}
...
def classify_strategy_outcome(record: dict[str, Any], previous_record: dict[str, Any] | None) -> tuple[str, str]:
    ...
    if not record.get("full_report_exists"):
...
def merge_strategy_constraints(run_memory: dict[str, Any], global_scope: dict[str, Any]) -> dict[str, list[str]]:
    ...
    def _is_blockable(fp: str, bucket: dict[str, Any]) -> bool:
        reason = str((bucket.get(fp) or {}).get("last_reason") or "")
        return not any(reason.startswith(inf) for inf in INFRA_FAILURE_REASONS)

    rejected_run = run_data.get("rejected") or {}
    blocked = set(fp for fp in rejected_run if _is_blockable(fp, rejected_run))
    preferred = set((run_data.get("positive") or {}).keys())

    rejected_global = global_scope.get("rejected") or {}
    blocked.update(fp for fp in rejected_global if _is_blockable(fp, rejected_global))
...
def load_manifest(manifest_path: Path, args: argparse.Namespace, run_dir: Path, solution_file: Path) -> dict[str, Any]:
    ...
    if manifest is None:
        manifest = {
            ...
            "reference_docs": BACKEND_REFERENCE_DOCS["cuda"],
            "preflight": {},
            "iterations": [],
            "best_iteration": None,
            "best_kernel_path": "",
        ...
        manifest["reference_docs"] = BACKEND_REFERENCE_DOCS["cuda"]
    return manifest
...
def resolve_solution_input(args: argparse.Namespace, manifest: dict[str, Any], cli_solution_file: Path) -> tuple[Path, str]:
    ...
    if mode in ("source", "explicit"):
        return cli_solution_file, mode
...
def build_benchmark_cmd(args: argparse.Namespace, benchmark_script: Path, snapshot_file: Path, benchmark_json: Path) -> list[str]:
    cmd = [
        sys.executable,
...
def render_final_summary(manifest: dict[str, Any]) -> str:
    ...
    lines.extend([
        ...
        "## Iterations",
        "",
        "| Iter | Strategy | Outcome | Reason | Correctness | Kernel median ms | Kernel avg ms | Full NCU | Snapshot |",
        "| --- | --- | --- | --- | --- | ---: | ---: | --- | --- |",
    ])
    ...
    for item in manifest.get("iterations", []):
        ...
        lines.append(
            "| v{iteration} | {strategy_fp} | {outcome} | {reason} | {correctness} | {median} | {avg} | {full_report} | {snapshot} |".format(
    ...
    if best_iteration is None:
        lines.append("- No eligible best version yet. Need benchmark success, correctness pass (if reference provided), and full NCU report.")
    else:
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(description="Evaluate one iteration in a CUDA kernel optimization loop")
    parser.add_argument("solution_file", help="Path to the current kernel file (.cu)")
    ...
    parser.add_argument("--resume-from", type=str, default="best", choices=["best", "source", "explicit"])
    ...
    parser.add_argument("--launch-skip", type=int, default=20)
    ...
    parser.add_argument("--kernel-name-regex", type=str, default="")
...
def main() -> int:
    ...
    benchmark_script = repo_root / "skills" / "kernel-opter-skill" / "profiling" / "script" / "benchmark.py"
    global_memory_file = repo_root / "skills" / "kernel-opter-skill" / "opt-loop" / "strategy-memory" / "global_strategy_memory.json"
    ...
    manifest = load_manifest(manifest_path, args, run_dir, cli_solution_file)

    canonical_source_file = Path(str(manifest.get("canonical_source_file") or cli_solution_file)).resolve()
    scope_key = build_scope_key(canonical_source_file, ref_file, list(args.dim_args), args.arch)
    ...
    preflight = collect_preflight(args, benchmark_script, source_for_iteration, ref_file)
    ...
    clock_ok, clock_msg = try_lock_gpu_clocks(args.gpu)
    print(f"[clock] {clock_msg}")
    manifest["preflight"]["clock_lock_ok"] = clock_ok
    manifest["preflight"]["clock_lock_msg"] = clock_msg
    ...
    snapshot_file = iter_dir / f"{source_for_iteration.stem}_v{iteration}.cu"
    ...
    bench_cmd = build_benchmark_cmd(args, benchmark_script, snapshot_file, benchmark_json)
    bench_res = run_command(bench_cmd, benchmark_stdout, benchmark_stderr)
    bench_json = read_json(benchmark_json, {})

    # For NCU profiling: skip compilation (already done above) to prevent
    # --target-processes from intercepting nvcc subprocess.
    ncu_bench_cmd = build_benchmark_cmd(args, benchmark_script, snapshot_file, benchmark_json)
    ncu_bench_cmd.append("--skip-compile")
    ...
    if not proposal_path.exists():
        ...
        write_text(proposal_path, proposal_stub.format(
    ...
    strategy_fingerprint = build_strategy_fingerprint(strategy_tags)

    previous_record = None
    if iteration > 0:
        previous_record = next(
            (item for item in manifest.get("iterations", []) if item.get("iteration") == iteration - 1),
            None,
        )
    ...
    if strategy_blocked:
        outcome = "rejected"
        reason = f"{base_reason};blocked_strategy_reused" if base_outcome == "rejected" else "blocked_strategy_reused"
